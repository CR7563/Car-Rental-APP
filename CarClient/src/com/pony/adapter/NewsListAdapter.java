...
public class NewsListAdapter extends BaseAdapter {
	...
	private List<MessageModel> list_result;
	private int posIndex;
	private Context mContext;

	public NewsListAdapter(Context context, List<MessageModel> list_result) {
		mContext = context;
...
//		try {
			holder.mTvTitle.setText(list_result.get(position).getNewsTitle());
			holder.mTvMoney.setText(list_result.get(position).getNewsContent());
			holder.mtvTime.setText(list_result.get(position).getNewsTime());
			
			
			if(!TextUtils.isEmpty(list_result.get(position).getNewsImage())){
				Picasso.with(mContext).load(Consts.URL_IMAGE+list_result.get(position).getNewsImage()).placeholder(R.drawable.default_drawable_show_pictrue)
				.into(holder.mivShop);
			}
